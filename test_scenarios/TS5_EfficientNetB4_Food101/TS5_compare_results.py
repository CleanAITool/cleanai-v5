"""
Test Scenario TS5 - Compare Results
...
def load_results():
    ...
    if not os.path.exists(results_file):
        ...
        print("  1. TS5_01_prepare_model.py")
        print("  2. TS5_02_coverage_pruning.py")
        print("  3. TS5_03_wanda_pruning.py")
        print("  4. TS5_04_magnitude_pruning.py")
        print("  5. TS5_05_taylor_pruning.py")
        return None
...
def print_summary_table(results):
    ...
    print("\n" + "="*110)
    print("TEST SCENARIO TS5: COMPREHENSIVE COMPARISON")
    print("="*110)
    ...
    if has_magnitude:
        change_data[0].append("Magnitude (TP)")
    if has_taylor:
        change_data[0].append("Taylor (TP)")
    ...
    if has_taylor:
        tay_time = results['taylor_pruning']['final']['inference_time_ms']
        time_change_row.append(f"{tay_time - orig_time:+.4f} ms ({(tay_time/orig_time - 1)*100:+.2f}%)")
    ...
    if has_flops and orig_flops > 0:
        ...
        if has_taylor:
            tay_flops = results['taylor_pruning']['final']['flops']
            flops_change_row.append(f"{(tay_flops - orig_flops)/1e9:+.2f} G ({(tay_flops/orig_flops - 1)*100:+.2f}%)")
    ...
    if len(methods_available) >= 2:
        # Find best method
        best_method = max(methods_available, key=lambda x: x[2])
        worst_method = min(methods_available, key=lambda x: x[2])
        
        print(f"\nBest Accuracy: {best_method[1]} ({best_method[2]:.2f}%)")
        print(f"Worst Accuracy: {worst_method[1]} ({worst_method[2]:.2f}%)")
        print(f"Accuracy Gap: {best_method[2] - worst_method[2]:.2f}%")
...
def print_individual_results(results):
    ...
    if 'magnitude_pruning' in results:
        ...
        print(f"After Pruning: {mag['pruned']['accuracy']:.2f}%")
        print(f"After Fine-tuning: {mag['final']['accuracy']:.2f}%")
        print(f"Pruning Ratio: {mag['pruning_config']['ratio']*100}%")
        print(f"Fine-tuning Epochs: {mag['final']['fine_tune_epochs']}")
    
    if 'taylor_pruning' in results:
        print("\n--- Taylor Gradient-Based Pruning ---")
        tay = results['taylor_pruning']
        print(f"Original Accuracy: {tay['original']['accuracy']:.2f}%")
        print(f"After Pruning: {tay['pruned']['accuracy']:.2f}%")
        print(f"After Fine-tuning: {tay['final']['accuracy']:.2f}%")
        print(f"Pruning Ratio: {tay['pruning_config']['ratio']*100}%")
        print(f"Fine-tuning Epochs: {tay['final']['fine_tune_epochs']}")
