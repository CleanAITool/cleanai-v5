...
Method: WANDA (Weight AND Activation) Pruning
Purpose: Apply WANDA pruning, then fine-tune to recover accuracy
...
def fine_tune_pruned_model(model: nn.Module, train_loader: DataLoader, test_loader: DataLoader) -> Dict:
    ...
    for epoch in range(1, FINE_TUNE_EPOCHS + 1):
        ...
        if epoch % SAVE_EVERY_N_EPOCHS == 0 or epoch == FINE_TUNE_EPOCHS:
            checkpoint_path = CHECKPOINT_DIR / f"{MODEL_NAME}_{DATASET_NAME}_FTAP_W_epoch{epoch}.pth"
            torch.save({
                'epoch': epoch,
...
def print_comparison_table(original_metrics: Dict, pruned_metrics: Dict, final_metrics: Dict):
    ...
    print("\n" + "="*100)
    print("WANDA PRUNING - COMPREHENSIVE COMPARISON TABLE")
    print("="*100)
...
def main():
    ...
    print("\n" + "#"*100)
    print(f"# TEST SCENARIO {TEST_SCENARIO} - SCRIPT 3: WANDA PRUNING + FINE-TUNING")
    print("#"*100)
    print(f"\nTest Scenario: {TEST_SCENARIO_NAME}")
    print(f"Method: {METHOD_NAME} (Weight AND Activation)")
    ...
    final_checkpoint_path = CHECKPOINT_DIR / f"{MODEL_NAME}_{DATASET_NAME}_FTAP_Wanda_final.pth"
    pruned_checkpoint_path = CHECKPOINT_DIR / f"{MODEL_NAME}_{DATASET_NAME}_pruned_Wanda.pth"
    ...
    if final_checkpoint_path.exists():
        ...
        # Step 4: Apply WANDA Pruning
        print("\n" + "="*60)
        print("Applying WANDA Pruning (Weight AND Activation)")
        print("="*60)
        print("WANDA combines weight magnitude with activation importance")
        print(f"Using {MAX_BATCHES} calibration batches from test set")
    ...
    print(f"✓ WANDA pruning experiment completed!")
