# Test Scenario TS2: ResNet-50 on ImageNet
...
## Scripts
1. **TS2_01_prepare_model.py**: Loads pretrained ResNet-50, evaluates on ImageNet validation
2. **TS2_02_coverage_pruning.py**: Applies Neuron Coverage pruning and fine-tunes
3. **TS2_03_wanda_pruning.py**: Applies Wanda pruning and fine-tunes
4. **TS2_compare_results.py**: Compares results from all methods
5. **TS2_run_all.py**: Runs all scripts in sequence
...
## Results
Results are saved to: `test_scenarios/TS2_ResNet50_ImageNet/TS2_Results.json`

## Performance
- **Baseline Accuracy**: ~76-77% (ResNet-50 on ImageNet)
- **Evaluation Time**: ~1-2 minutes (50k images)
- **Pruning Time**: ~5-10 minutes
- **Fine-tuning**: ~10-20 minutes (5 epochs)
