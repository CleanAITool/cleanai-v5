"""
Test Scenario TS7: FP16 Quantization Analysis
=============================================

This script evaluates FP16 quantization on all TS7 pruned models:
...
CONFIG = {
    ...
    'results_file': 'TS7_Quantization_Results.json',
    'batch_size': 32,
    'num_workers': 4,
...
MODELS_TO_EVALUATE = {
    'Original_FT': {
        'checkpoint': 'ResNet50_ImageNet_FT_best.pth',
        'description': 'Original Fine-tuned ResNet50',
        'is_pruned': False,
    },
    'Coverage_Pruned_FT': {
        'checkpoint': 'ResNet50_ImageNet_FTAP_NC_best.pth',
        'description': 'Coverage Pruning + Fine-tuned',
        'is_pruned': True,
    },
    'Wanda_Pruned_FT': {
        'checkpoint': 'ResNet50_ImageNet_FTAP_W_best.pth',
        'description': 'Wanda Pruning + Fine-tuned',
        'is_pruned': True,
    },
    'Magnitude_Pruned_FT': {
        'checkpoint': 'ResNet50_ImageNet_FTAP_MAG_best.pth',
        'description': 'Magnitude Pruning + Fine-tuned',
        'is_pruned': True,
    },
    'Taylor_Pruned_FT': {
        'checkpoint': 'ResNet50_ImageNet_FTAP_TAY_best.pth',
...
# ===================== DATA LOADING =====================
def get_imagenet_loader(batch_size=32, num_workers=4, num_samples=None):
    ...
    if is_pruned:
        ...
        if 'model' in checkpoint:
            ...
            raise RuntimeError(
                ...
                f"Pruned models have modified architecture that cannot be loaded into base ResNet50.\n"
                f"Please re-run the pruning script to save the full model with 'model' key."
            )
    else:
        # For non-pruned models, create base model and load state dict
...
def run_quantization_comparison():
    ...
    print("\n" + "="*80)
    print("TS7 FP16 QUANTIZATION ANALYSIS")
    print("="*80)
    
    # Check dataset
    val_dir = os.path.join(CONFIG['dataset_dir'], 'val')
    if not os.path.exists(val_dir):
        print(f"\nERROR: ImageNet validation dataset not found at: {val_dir}")
        print("Please download ImageNet validation set first.")
        return
    
    # Load data
    data_loader = get_imagenet_loader(
...
if __name__ == '__main__':
    ...
    print("\n" + "#"*80)
    print("# TS7 Quantization Analysis Complete!")
    print("#"*80 + "\n")
